...
#Reading the file
s3_path = 's3://etl-project-spotify/raw_data/to_processed/'
...
source_df.show()
#Converting the JSON file into DF to easily process it
...
def write_to_s3(df, path_suffix, format_type="csv"): #path_suffix: s3 path to store the data
    # Convert Spark Dataframe back to AWS Glue DynamicFrame as it has the capability to write directly into s3
    dynamic_frame = DynamicFrame.fromDF(df, glueContext, "dynamic_frame")
    
    glueContext.write_dynamic_frame.from_options(
        frame = dynamic_frame,
        connection_type = "s3",
        connection_options = {"path": f"s3://etl-project-spotify/transformed_data/{path_suffix}/"},
        format = format_type
    ) 
#write data to s3   
...
#Transferring files from to_processed folder to processed folder
...
bucket_name = "etl-project-spotify"
prefix = "raw_data/to_processed/"
spotify.keys = list_s3_objects(bucket_name, prefix)
